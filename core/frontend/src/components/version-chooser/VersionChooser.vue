<template>
  <div>
    <v-col max-width="1000" class="mx-auto">
      <v-dialog v-model="show_docker_login_dialog" max-width="450">
        <DockerLogin @cancel="show_docker_login_dialog = false" />
      </v-dialog>
      <v-card
      ...
      >
        <v-alert border="top" colored-border type="info" elevation="2">
          ...
          <!--! ===== Changed the names ===== -->
          Turn on Pirate mode to view all available CoratiaOS versions,
          including previous installs stored onboard, as well as past and
          current stable, beta, and development releases downloadable from
        ...
        <version-card
          ...
          :up-to-date="
            ...
            !newStableAvailable() &&
            !newBetaAvailable() &&
            !updateIsAvailable(current_version)
          "
        ...
        />
        <v-alert v-if="available_versions.error" color="red lighten-2" dark>
      ...
      >
        ...
        <version-card
          ...
          :current="
            image.tag === current_version?.tag &&
            image.repository === current_version?.repository
          "
      ...
      >
        <div class="d-flex justify-space-between pb-3">
          ...
          <v-btn color="primary" @click="show_docker_login_dialog = true">
            Docker Login
          </v-btn>
        </div>
        <v-form @submit.prevent="loadVersions()">
          <v-text-field
            v-model="selected_image"
            name="BlueOS Remote Repository"
            label="Remote repository"
            :append-icon="
              selected_image != default_repository ? 'mdi-restore' : undefined
            "
        ...
        <version-card
          ...
          @pull-and-apply="pullAndSetVersion"
        />
        <v-card-actions v-if="!loading_images" class="justify-center">
          <v-pagination
            v-if="totalPages > 1"
            v-model="page"
            :length="totalPages"
    ...
    />
    <v-card v-if="settings.is_pirate_mode" max-width="900" class="mx-auto pa-4">
      <h2>Manual upload</h2>
      ...
      <v-alert
        ...
        elevation="2"
      >
        Decompressing file, this usually takes up to one minute, please stand
        by...
        <spinning-logo
          v-if="upload_percentage == 100"
          size="15%"
    ...
    <v-overlay :z-index="10" :value="waiting">
      <spinning-logo size="100%" subtitle="Switching to new core..." />
    </v-overlay>
  </div>
</template>
...
export default Vue.extend({
  ...
  computed: {
    paginatedComponents(): Version[] {
      return this.available_versions.remote.slice(
        (this.page - 1) * 10,
        this.page * 10,
      );
    },
  ...
  methods: {
    ...
    newBetaAvailable() {
      ...
      if (
        ...
        this.latest_beta !== this.current_version.tag
      ) {
    ...
    newStableAvailable() {
      ...
      if (
        ...
        sem_ver_greater(
          new SemVer(this.latest_stable),
          new SemVer(this.current_version.tag),
        )
      ) {
    ...
    async loadAvailableVersions() {
      ...
      await VCU.loadAvailableVersions(this.selected_image)
        ...
        .finally(() => {
          ...
          this.latest_stable = VCU.getLatestStable(
    ...
    async loadVersions() {
      Promise.all([this.loadLocalVersions(), this.loadAvailableVersions()]);
    },
    ...
    updateIsAvailable(image: Version) {
      ...
      return (
        remote_counterpart.sha !== image.sha && remote_counterpart.sha !== null
      );
    },
    async upload() {
    ...
    async deleteVersion(args: string | string[]) {
      ...
      await back_axios({
        ...
        .then(() => {
          ...
          this.local_versions.result.local =
            this.local_versions.result.local.filter(
              (element) =>
                element.repository !== repository || element.tag !== tag,
            );
        })
        .catch((error) => {
