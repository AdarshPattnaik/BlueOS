<template>
  <v-card class="ma-2 pa-2">
    <v-card-title class="align-center"> Battery Monitor </v-card-title>
    <v-card-text>
      ...
      <div v-else>
        <span class="d-block"
          ><b>Voltage Pin:</b> {{ printParam(batt_monitor.vPin) }}</span
        >
        <span class="d-block"
          ><b>Voltage Multiplier:</b>
          {{ printParam(batt_monitor.vMulti) }}</span
        >
        <span class="d-block"
          ><b>Current Pin:</b> {{ printParam(batt_monitor.aPin) }}</span
        >
        <span class="d-block"
          ><b>Current per Volt:</b> {{ printParam(batt_monitor.aPv) }}</span
        >
        <span class="d-block"
          ><b>Current Offset:</b> {{ printParam(batt_monitor.aOff) }}</span
        >
      </div>
      <span class="d-block"
        ><b>Low Voltage Failsafe:</b>
        {{ printParam(batt_failsafe_low.action) }}</span
      >
      <span v-if="batt_failsafe_low.level?.value" class="d-block">
        <b>Low Voltage Level:</b> {{ printParam(batt_failsafe_low.level) }}
      </span>
      <span class="d-block"
        ><b>Critical Voltage Failsafe:</b>
        {{ printParam(batt_failsafe_critical.action) }}</span
      >
      <span v-if="batt_failsafe_critical.level?.value" class="d-block">
        <b>Low Voltage Level:</b> {{ printParam(batt_failsafe_low.level) }}
      </span>
    </v-card-text>
  </v-card>
  ...
  computed: {
    ...
    is_known_power_meter(): string | false {
      for (const entry of power_modules_list) {
        if (
          entry.voltPin === this.batt_monitor.vPin?.value &&
    ...
    battery_voltage(): string {
      const voltage_microvolts = mavlink_store_get(
        mavlink,
        "SYS_STATUS.messageData.message.voltage_battery",
      ) as number;
      return ((voltage_microvolts as number) / 1000).toFixed(2);
    },

    battery_current(): string {
      const current_centiampere = mavlink_store_get(
        mavlink,
        "SYS_STATUS.messageData.message.current_battery",
      ) as number;
