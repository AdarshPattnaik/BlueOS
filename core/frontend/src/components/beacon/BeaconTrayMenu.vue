    ...
    <v-card elevation="1" width="300">
      <v-container>
        <div class="justify-center">
          ...
          <!--! ===== Changed the name ===== -->
          It looks like you are reaching CoratiaOS via its wi-fi network. This
          ...
          >
            ...
            <v-btn small :href="`http://${available_wired_domain()}.local`">
  ...
  computed: {
    wired_interface_domains(): Domain[] {
      return beacon.available_domains.filter(
        (entry) => entry.interface_type === InterfaceType.WIRED
          || entry.interface_type === InterfaceType.USB,
      )
    },
    wireless_interface_domains(): Domain[] {
      return beacon.available_domains.filter(
        (entry) => entry.interface_type === InterfaceType.WIFI
          || entry.interface_type === InterfaceType.HOTSPOT,
      )
    },
    is_connected_to_wired(): boolean {
      return this.wired_interface_domains.some(
        (domain) => domain.ip === beacon.nginx_ip_address,
      )
    },
    is_connected_to_wifi(): boolean {
      const is_on_wifi = this.wireless_interface_domains.some(
        (domain) => domain.ip === beacon.nginx_ip_address,
      )

      if (is_on_wifi && this.is_connected_to_wired) {
        console.debug(
          'Unexpected behavior. There are both Wired and Wireless interfaces sharing the same IP address.',
        )
      }
    ...
    should_warn_user(): boolean {
      return (
        this.is_connected_to_wifi || this.is_connected_to_unknown_interface
      )
    },
    tooltip_text(): string {
      ...
      return (
        'It looks like you are reaching BlueOS using a complex network. '
