  ...
  methods: {
    ...
    async disableWifiHotspot(): Promise<ConfigurationStatus> {
      return back_axios({
        ...
        .then(() => undefined)
        .catch(
          (error) => `Failed to disable wifi hotspot: ${error.message ?? error.response?.data}.`,
        )
    },
    async disableSmartWifiHotspot(): Promise<ConfigurationStatus> {
      return back_axios({
        ...
        .then(() => undefined)
        .catch(
          (error) => `Failed to disable smart wifi hotspot: ${error.message ?? error.response?.data}.`,
        )
    },
      ...
      return availableFirmwares(vehicle)
        .then((firmwares: Firmware[]) => {
          ...
          return installFirmwareFromUrl(found.url, true, this.params)
            .then(() => undefined)
            .catch(
              (error) => `Failed to install firmware: ${error.message ?? error.response?.data}.`,
            )
        })
        .catch(
          (error) => `Failed to fetch available firmware: ${error.message ?? error.response?.data}.`,
        )
    },
    async installScripts(): Promise<ConfigurationStatus> {
      ...
      try {
        ...
        await Promise.allSettled(
          this.scripts.map(async (script) => filebrowser.createFile(
