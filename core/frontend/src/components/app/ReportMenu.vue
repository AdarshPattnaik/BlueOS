<template>
  <v-container id="feature-request-button" class="d-flex justify-center">
    <v-btn class="mr-2" icon large elevation="2" @click="showDialog(true)">
      <v-icon>mdi-bug</v-icon>
    </v-btn>
    <v-dialog width="fit-content" :value="show_dialog" @input="showDialog">
      <v-card>
        ...
        <v-container class="pa-2">
          <v-card-actions class="flex-column">
            <v-btn
              v-tooltip="
                'Track changes, contribute, and get notified when fixed'
              "
              class="ma-2"
              @click="openGitHub()"
            >
            ...
            >
              <v-icon left size="20"> mdi-message-text </v-icon>
              With Simple Report
            </v-btn>
            ...
            <v-btn
              ...
              @click="openDiscuss()"
            >
              <v-icon left size="16"> $si-discourse </v-icon>
...
export default Vue.extend({
  ...
  methods: {
    ...
    openGitHub(): void {
      window.open(
        "https://github.com/bluerobotics/BlueOS/issues/new/choose",
        "_blank",
      );
    },
    openDiscuss(): void {
    ...
    async openSimpleReport(): Promise<void> {
      ...
      const feedback = Sentry.feedbackIntegration({
        ...
        messagePlaceholder:
          "What's the bug? What did you expect? What feedback do you have?",
        themeLight: theme,
        themeDark: theme,
      });
